namespace Creatio.Copilot
{
	...
	internal class CopilotEngine : ICopilotEngine
	{
		...
		private string RemoveInvalidLinks(string content, bool isApi = false) {
			try {
				string prompt = isApi ? ApiPromptToRemoveInvalidLinks : PromptToRemoveInvalidLinks;
				ChatCompletionRequest rewriteRequest = CreateCompletionRequest(prompt, content);
		...
		private IEnumerable<CopilotActionMetaItem> GetCurrentIntentActionsMetaItems(Guid? intentId) {
			...
			if (intent?.Type == CopilotIntentType.Skill && !IntentSchemaService.HasOperationPermitted(
		...
		private void AddDocumentsCompletionMessages(CopilotSession session, List<ChatMessage> messages) {
			if (!Features.GetIsEnabled<GenAIFeatures.UseFileHandling>() || session.Documents.IsNullOrEmpty()) {
				return;
			}
			var completionMessages = _documentTool.GetDocumentMessagesForCompletion(session.Documents)
				.Select(msg => msg.ToCompletionApiMessage());
		...
		private CopilotMessage GetAgentsDescriptionMessage(IEnumerable<CopilotIntentSchema> intents) {
			...
			var agentsWithSubIntents = intents
				...
				.Select(agent => {
					var subIntents = IntentSchemaService.GetSubIntents(agent.UId)
						.Select(i => i.Caption.Value)
						.ToList();
					return new {
						agent.Name,
		...
		private async Task<Guid> CompleteIntentAsync(CopilotSession session, CopilotIntentCallResult response,
			...
			ChatCompletionRequest completionRequest = CreateCompletionRequest(session, copilotToolContext);
			try {
			...
			if (isToolCallsEnabled) {
				await HandleApiToolCallsAsync(session, completionResponse, copilotToolContext.Mapping, response, token);
			}
			return requestId;
		}
		...
		private void MapSkillResponse(ChatCompletionResponse completionResponse,
			...
			if (Features.GetIsEnabled<GenAIFeatures.EnableApiLinkValidation>()) {
				assistantMessages.ForEach(message => {
					...
					message.Content = RemoveInvalidLinks(markedContent, true);
				});
			}
			session.AddMessages(assistantMessages);
		...
		private string GenerateIntentPrompt(IDictionary<string, object> parameterValues, string additionalPromptText,
			CopilotIntentSchema intent, IList<string> warnings) {
		...
		private void FillIntentDocuments(CopilotSession session) {
		...
		public async Task<CopilotIntentCallResult> ExecuteIntentAsync(CopilotIntentCall call,
			...
			string prompt = GenerateIntentPrompt(call.Parameters, call.AdditionalPromptText, intent, response.Warnings);
